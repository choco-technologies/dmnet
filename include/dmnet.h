...
#include "dmnet_defs.h"

/**
 * Public API for the dmnet module.
 *
 * Functions are declared with the dmod_dmnet_api(...) macro - dmod's
 ...
 *
 * Definitions in src/dmnet.c use the matching
 * dmod_dmnet_api_declaration(...) macro - a plain C function
 ...
 * src/dmnet.c with your module's real API.
 */

/* Opaque handle - the real struct is defined in src/dmnet.c */
typedef struct dmnet* dmnet_t;

/**
 * Create a new dmnet instance.
 *
 * @return A valid handle on success, or NULL on allocation failure.
 */
dmod_dmnet_api(1.0, dmnet_t, _create, ( void ));

/**
 * Destroy an instance created by dmnet_create(). Safe to call with
 * NULL.
 */
dmod_dmnet_api(1.0, void, _destroy, ( dmnet_t handle ));
 ...
 */
dmod_dmnet_api(1.0, bool, _is_valid, ( dmnet_t handle ));
