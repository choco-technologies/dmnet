...
DMOD_TEST_STEP(ip_address_starts_unassigned)
{
    ...
    DMOD_TEST_EXPECT_EQ(ip.family, dmip_family_none);
}

DMOD_TEST_STEP(set_and_get_ipv4_address_roundtrips)
{
    dmip_addr_t set_ip = { 0 };
    set_ip.family = dmip_family_v4;
    ...
    dmip_addr_t got_ip = { 0 };
    DMOD_TEST_EXPECT_EQ(dmnetif_get_ip_address(g_iface, &got_ip), 0);
    DMOD_TEST_EXPECT_EQ(got_ip.family, dmip_family_v4);
    DMOD_TEST_EXPECT_TRUE(bytes_equal(got_ip.addr.v4, set_ip.addr.v4, DMIP_IPV4_ADDR_LEN));
}

DMOD_TEST_STEP(set_and_get_ipv6_address_roundtrips)
{
    dmip_addr_t set_ip = { 0 };
    set_ip.family = dmip_family_v6;
    for (int i = 0; i < DMIP_IPV6_ADDR_LEN; i++)
    {
    ...
    dmip_addr_t got_ip = { 0 };
    DMOD_TEST_EXPECT_EQ(dmnetif_get_ip_address(g_iface, &got_ip), 0);
    DMOD_TEST_EXPECT_EQ(got_ip.family, dmip_family_v6);
    DMOD_TEST_EXPECT_TRUE(bytes_equal(got_ip.addr.v6, set_ip.addr.v6, DMIP_IPV6_ADDR_LEN));
}

DMOD_TEST_STEP(set_ip_address_none_clears_it)
{
    dmip_addr_t v4_ip = { 0 };
    v4_ip.family = dmip_family_v4;
    DMOD_TEST_EXPECT_EQ(dmnetif_set_ip_address(g_iface, &v4_ip), 0);

    dmip_addr_t clear_ip = { 0 };
    clear_ip.family = dmip_family_none;
    DMOD_TEST_EXPECT_EQ(dmnetif_set_ip_address(g_iface, &clear_ip), 0);

    dmip_addr_t got_ip = { 0 };
    DMOD_TEST_EXPECT_EQ(dmnetif_get_ip_address(g_iface, &got_ip), 0);
    DMOD_TEST_EXPECT_EQ(got_ip.family, dmip_family_none);
}

DMOD_TEST_STEP(set_ip_address_invalid_family_fails)
{
    dmip_addr_t bad_ip = { 0 };
    bad_ip.family = (dmip_family_t)7;
...
DMOD_TEST_STEP(netmask_starts_unassigned)
{
    ...
    DMOD_TEST_EXPECT_EQ(netmask.family, dmip_family_none);
}

DMOD_TEST_STEP(set_and_get_ipv4_netmask_roundtrips)
{
    dmip_addr_t set_netmask = { 0 };
    set_netmask.family = dmip_family_v4;
    ...
    dmip_addr_t got_netmask = { 0 };
    DMOD_TEST_EXPECT_EQ(dmnetif_get_netmask(g_iface, &got_netmask), 0);
    DMOD_TEST_EXPECT_EQ(got_netmask.family, dmip_family_v4);
    DMOD_TEST_EXPECT_TRUE(bytes_equal(got_netmask.addr.v4, set_netmask.addr.v4, DMIP_IPV4_ADDR_LEN));
}

DMOD_TEST_STEP(set_and_get_ipv6_netmask_roundtrips)
{
    dmip_addr_t set_netmask = { 0 };
    set_netmask.family = dmip_family_v6;
    for (int i = 0; i < DMIP_IPV6_ADDR_LEN; i++)
    {
    ...
    dmip_addr_t got_netmask = { 0 };
    DMOD_TEST_EXPECT_EQ(dmnetif_get_netmask(g_iface, &got_netmask), 0);
    DMOD_TEST_EXPECT_EQ(got_netmask.family, dmip_family_v6);
    DMOD_TEST_EXPECT_TRUE(bytes_equal(got_netmask.addr.v6, set_netmask.addr.v6, DMIP_IPV6_ADDR_LEN));
}

DMOD_TEST_STEP(set_netmask_none_clears_it)
{
    dmip_addr_t v4_netmask = { 0 };
    v4_netmask.family = dmip_family_v4;
    DMOD_TEST_EXPECT_EQ(dmnetif_set_netmask(g_iface, &v4_netmask), 0);

    dmip_addr_t clear_netmask = { 0 };
    clear_netmask.family = dmip_family_none;
    DMOD_TEST_EXPECT_EQ(dmnetif_set_netmask(g_iface, &clear_netmask), 0);

    dmip_addr_t got_netmask = { 0 };
    DMOD_TEST_EXPECT_EQ(dmnetif_get_netmask(g_iface, &got_netmask), 0);
    DMOD_TEST_EXPECT_EQ(got_netmask.family, dmip_family_none);
}

DMOD_TEST_STEP(set_netmask_invalid_family_fails)
{
    dmip_addr_t bad_netmask = { 0 };
    bad_netmask.family = (dmip_family_t)7;
...
DMOD_TEST_STEP(broadcast_starts_unassigned)
{
    ...
    DMOD_TEST_EXPECT_EQ(broadcast.family, dmip_family_none);
}

DMOD_TEST_STEP(set_and_get_ipv4_broadcast_roundtrips)
{
    dmip_addr_t set_broadcast = { 0 };
    set_broadcast.family = dmip_family_v4;
    ...
    dmip_addr_t got_broadcast = { 0 };
    DMOD_TEST_EXPECT_EQ(dmnetif_get_broadcast(g_iface, &got_broadcast), 0);
    DMOD_TEST_EXPECT_EQ(got_broadcast.family, dmip_family_v4);
    DMOD_TEST_EXPECT_TRUE(bytes_equal(got_broadcast.addr.v4, set_broadcast.addr.v4, DMIP_IPV4_ADDR_LEN));
}

DMOD_TEST_STEP(set_and_get_ipv6_broadcast_roundtrips)
{
    dmip_addr_t set_broadcast = { 0 };
    set_broadcast.family = dmip_family_v6;
    for (int i = 0; i < DMIP_IPV6_ADDR_LEN; i++)
    {
    ...
    dmip_addr_t got_broadcast = { 0 };
    DMOD_TEST_EXPECT_EQ(dmnetif_get_broadcast(g_iface, &got_broadcast), 0);
    DMOD_TEST_EXPECT_EQ(got_broadcast.family, dmip_family_v6);
    DMOD_TEST_EXPECT_TRUE(bytes_equal(got_broadcast.addr.v6, set_broadcast.addr.v6, DMIP_IPV6_ADDR_LEN));
}

DMOD_TEST_STEP(set_broadcast_none_clears_it)
{
    dmip_addr_t v4_broadcast = { 0 };
    v4_broadcast.family = dmip_family_v4;
    DMOD_TEST_EXPECT_EQ(dmnetif_set_broadcast(g_iface, &v4_broadcast), 0);

    dmip_addr_t clear_broadcast = { 0 };
    clear_broadcast.family = dmip_family_none;
    DMOD_TEST_EXPECT_EQ(dmnetif_set_broadcast(g_iface, &clear_broadcast), 0);

    dmip_addr_t got_broadcast = { 0 };
    DMOD_TEST_EXPECT_EQ(dmnetif_get_broadcast(g_iface, &got_broadcast), 0);
    DMOD_TEST_EXPECT_EQ(got_broadcast.family, dmip_family_none);
}

DMOD_TEST_STEP(set_broadcast_invalid_family_fails)
{
    dmip_addr_t bad_broadcast = { 0 };
    bad_broadcast.family = (dmip_family_t)7;
...
{
    dmip_addr_t ip = make_v4(10, 5, 5, 5);
...
DMOD_TEST_STEP(assigning_ip_with_netmask_adds_subnet_route)
{
    ...
    dmip_addr_t ip = make_v4(192, 168, 1, 42);
    DMOD_TEST_EXPECT_EQ(dmnetif_set_ip_address(g_iface, &ip), 0);

    dmip_addr_t other_host = make_v4(192, 168, 1, 200);
...
{
    dmip_addr_t ip = make_v4(10, 8, 8, 8);
    DMOD_TEST_EXPECT_EQ(dmnetif_set_ip_address(g_iface, &ip), 0);
    DMOD_TEST_EXPECT_NOT_NULL(dmroute_lookup(&ip));

    dmip_addr_t none = { 0 };
    none.family = dmip_family_none;
...
{
    dmip_addr_t first_ip = make_v4(10, 1, 1, 1);
    ...
    dmip_addr_t second_ip = make_v4(10, 2, 2, 2);
...
{
    dmip_addr_t ip = make_v4(10, 9, 9, 9);
    DMOD_TEST_EXPECT_EQ(dmnetif_set_ip_address(g_iface, &ip), 0);
    DMOD_TEST_EXPECT_NOT_NULL(dmroute_lookup(&ip));
