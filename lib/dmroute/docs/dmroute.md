...
Note the direction: dmroute sits *below* dmnetif here, not on top of it -
dmnetif is the one with a dependency on dmroute (see "Automatic
registration" below), and dmroute has no dependency on dmnetif at all. A
...
- If the address is cleared (`dmip_family_none`), dmnetif calls
...
- Route addresses use `dmip_addr_t` (see [dmip](../../dmip)), a small
  standalone type with the same shape as dmnetif's address fields, rather
  than reusing a dmnetif type directly.
...
Like `dmip_addr_t` itself, a route's destination/netmask/gateway share one
type discriminated by `family` rather than separate v4/v6 types - a route
entry, a lookup, and the prefix-length/masking math all branch on `family`
once rather than needing parallel code paths.

## Dependencies

- `dmip` - the shared `dmip_addr_t` type every route field uses
