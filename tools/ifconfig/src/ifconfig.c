...
/* Parses "A.B.C.D" (each octet 0-255) into an IPv4 dmnetif_ip_addr_t. */
static bool parse_ipv4_address(const char* s, dmnetif_ip_addr_t* ip)
{
    if (s == NULL)
        return false;
    ...
    for (int i = 0; i < DMNETIF_IPV4_ADDR_LEN; i++)
    {
        if (*s < '0' || *s > '9')
            return false;
...
static void print_ipv4_address(const dmnetif_ip_addr_t* ip)
{
...
static void print_ipv6_address(const dmnetif_ip_addr_t* ip)
{
...
static void print_ip_address(dmnetif_iface_t iface, const dmnetif_ip_addr_t* ip)
{
    if (ip->family == dmnetif_ip_family_v4)
    {
        ...
        dmnetif_ip_addr_t netmask;
        if (dmnetif_get_netmask(iface, &netmask) == 0 && netmask.family == dmnetif_ip_family_v4)
        {
            Dmod_Printf("  netmask ");
            print_ipv4_address(&netmask);
        }

        dmnetif_ip_addr_t broadcast;
        if (dmnetif_get_broadcast(iface, &broadcast) == 0 && broadcast.family == dmnetif_ip_family_v4)
        {
